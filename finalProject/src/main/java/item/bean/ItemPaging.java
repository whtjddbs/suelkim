...
public class ItemPaging {
	...
	public void makePagingHTML(String main_codename) {
		...
		if(startPage!=1)
			pagingHTML.append("[<a href='newDisplay.do?main_codename="+main_codename+"&pg="+(startPage-1)+"'><</a>]");
		...
		if(endPage<totalP)
			pagingHTML.append("[<a href='newDisplay.do?main_codename="+main_codename+"&pg="+(endPage+1)+"'>></a>]");
	}
}
